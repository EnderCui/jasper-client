...
def getPiStatus(logger, mic):
    ...
    result['ram_percentage'] = result['ram_used'] * 100 / result['ram_total']
...
def handle(text, mic, profile, bot=None):
    ...
    try:
        ...
        mic.say(u'处理器温度' + str(status['cpu_tmp']) +
                u'度,内存使用百分之' + str(status['ram_percentage']) +
                u',存储使用百分之' + str(status['disk_percentage']))
    except Exception, e:
        logger.error(e)
